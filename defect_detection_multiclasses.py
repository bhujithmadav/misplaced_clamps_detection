...
class ImageInput(BaseModel):
    ...
    def validate_input_text(cls, value):
        allowed_values = {'fire_extinguisher','grid','layout','power_eco','top'}
        if value.lower() not in allowed_values:
            error = "Input should be one among the following ['fire_extinguisher','grid','layout','power_eco','top']"
            raise HTTPException(status_code=422, detail=str(error))
            #raise ValueError("Input should be either 'top' or 'pe'.")
        return value.lower()
